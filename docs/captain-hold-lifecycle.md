...
`hold --park` records parked or standby execution state through tasks-axi `--kind parked` instead of `--kind captain`: it writes no hold-set stamp, publishes no new needs-decision, and writes a `state/<task-id>.parked` marker that `bin/fm-classify-lib.sh`'s `scan_open_decisions`/`scan_open_decisions_incremental` use to exclude the task from the OPEN DECISIONS drain while leaving its underlying status log untouched.
Parking a task that already carried a live, unresolved captain hold publishes that occurrence's `resolved [key=captain-hold-<id>-<n>]: ... parked` event to the parent channel instead of leaving the call dangling; a fresh park with no prior live hold publishes nothing.
That publish attempt can itself fail (a broken or unreachable parent route) after the backlog has already transitioned to `hold_kind: parked`, at which point `existing_hold_kind` alone can no longer tell a fresh park from one still owing its resolution: a durable `state/<id>.park-pending-resolve` marker, written before the attempt and removed only once `publish_parent_hold` confirms delivery, is what a later `hold --park` retry checks instead, so the deferred resolution is not silently lost.
An ordinary `hold` without `--park` on a previously parked task removes the `.parked` marker and resumes the live path.
...
`tests/fm-captain-hold-park.test.sh` pins `hold --park`: a fresh parked item records `hold_kind: parked`, writes no hold-set stamp, and stays out of the OPEN DECISIONS drain even while its raw status log still carries an unhandled needs-decision line; an ordinary held item still surfaces normally; un-parking removes the marker and restores hold_kind: captain; and, exercised through a secondmate home where the effect is directly observable, parking a task that already carried a live unresolved captain hold publishes that occurrence's `resolved ... parked` event to the parent channel while a fresh park with nothing to resolve publishes nothing.
The same suite proves the retry path for a failed publish: making the parent channel file unwritable during a park forces `publish_parent_hold` to fail, after which the backlog still shows `hold_kind: parked`, no resolved line lands, and a `state/<id>.park-pending-resolve` marker survives; restoring write access and re-running the same `hold --park` then completes the deferred resolution and clears the marker.
